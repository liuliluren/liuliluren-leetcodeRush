...
> 核心思路
>
> 实现上有点小问题，没法处理最开始就是小的情况。没有去抓🐛了。
>
> ```java
> class Solution {
>     public boolean searchMatrix(int[][] matrix, int target) {
>         int[] a = Arrays.stream(matrix) // 将二维数组转换为 Stream<int[]>
>                 .mapToInt(row -> row[0]) // 对每个一维数组 (行) 取第一个元素，转换为 IntStream
>                 .toArray(); // 将 IntStream 转换为 int[] 数组
>         int x = midSearch(a, (a.length - 1) /2, target);
>         int y = midSearch(matrix[x], (matrix[x].length - 1) /2, target);
>         if (matrix[x][y] == target || (y < matrix[x].length - 1 && matrix[x][y + 1] == target)) {
>             return true;
>         }
>         return false;
>     }
> //    输入中间值，查找目标值所在的行
>     public int midSearch(int[] targetMatrix, int mid, int target){
>         if (targetMatrix[mid] <= target){
>             if (mid == targetMatrix.length - 1 || (mid < targetMatrix.length - 1 && target <= targetMatrix[mid + 1])){
>                 return mid;
>             }else {
>                 return midSearch(targetMatrix, (targetMatrix.length - 1 + mid) /2 + 1, target);
>             }
>         }else
>             return midSearch(targetMatrix, mid / 2, target);
>     }
> }
> ```
>
...
> 核心思路
>
> 思路很容易想到，但是二分查找的边界问题比较麻烦。选择直接基于官方题解的处理方式。
...
> 核心思路
>
> ⭐该二分法的实现好好记一记，其提供了lower选项，能查找目标数组中存在多个target数的情况，并给出其中最左或者最右的位置数的位置。此外，这里返回的ans是从1开始计数的，将`return ans`修改为`return ans-1`即可返回从0开始计数的结果。
>
> ```java
> public int binarySearch(int[] nums, int target, boolean lower) {
>     int left = 0, right = nums.length - 1, ans = nums.length;
...
> ```
>
> 另外， 这里边的`(lower && nums[mid] >= target)`替换为`(lower && nums[mid] == target)`也是能行的。
...
> /**
>  * 设置head tail两个指针
>  * 若干种情况可能（'>' means 'target > head | tail'）
>  * 1: > >
>  *  target在head和tail之间，或者tail~size()-1之间，即target只能在head~size()-1之间
>  *  head ~ target ~ size()-1
>  *  比较mid以进行下一步查找；
>  * 2: > <
>  *  target在head和tail中间
>  *  head ~ target ~ tail
>  *  变成传统的二分查找，即当前区间内是个单调递增的排列，使用普通的二分查找即可；
>  * 3: < >
>  *  target在0~head或者tail~size()-1之间
>  *  0 ~ target ~ head || tail ~ target ~ size()-1
>  *  对两个位置的区间进行下一步查找；
>  * 4: < <
>  *  target在0~head之间，或者在head~tail之间，即target在0~tail之间
>  *  0 ~ target ~ tail
>  *  比较mid以进行下一步查找；
>  */
> class Solution {
...
>         return res;
>     }
>     public void binarySearch(int[] nums, int target, int head, int tail) {
>         if (head < 0 || tail == length || head == length || tail < 0) {
>             return;
>         }
>         if (target == nums[head]) {
>             res = head;
...
>                 binarySearch(nums, target, 0 , head - 1);
>                 binarySearch(nums, target, tail + 1 , length - 1);
>             }else {
>                 if (target < nums[mid]) {
>                     binarySearch(nums, target, head + 1, mid - 1);
>                 }else {
>                     binarySearch(nums, target, mid + 1, tail - 1);
>                 }
>             }
>         }else {
>             if (target > nums[head]){
>                 if (target > nums[mid] && nums[mid] > nums[tail]) {
>                     binarySearch(nums, target, mid + 1, tail - 1);
>                 }else {
>                     binarySearch(nums, target, head + 1, mid - 1);
>                 }
...
> /**
>  * 不应该使用太多的分类情况进行讨论
>  * 猜测使用offset来转变成传统的二分查找
>  * 首先需要找到offset值，然后使用0 + offset作为新的head，使用（offset - 1 + length） % length作为tail
>  * 以{4,5,6,7,0,1,2}为例，进行了4次旋转，因此offset = 4，head = offset = 4；tail = （offset - 1 + length） % length = 3
>  * 确认新的head和tail之后，进行“偏移后的二分查找”
>  * mid = (((head - offset + length) % length + (tail - offset + length) % length) / 2 + offset + length) % length
>  */
> class Solution {
>     public int search(int[] nums, int target) {
...
>             }
>         }
>         int head = end, tail = start, offset = end;
...
>         }
>         // 找到offset
>         while((tail - offset + length) % length - (head - offset + length) % length > 0){
>             if (nums[head] == target){
>                 return head;
>             }
>             if (nums[tail] == target){
>                 return tail;
>             }
>             int mid = (((head - offset + length) % length + (tail - offset + length) % length) / 2 + offset + length) % length;
>             if (nums[mid] < target){
>                 head = mid;
>             }else {
>                 tail = mid;
...
> 先取mid。
>
> 经由mid分割成左右两组排列之后，**必定（至少）有一边是有序的排列。**
>
> 判断target的大小是否落于有序排列的范围中，如果在的话就转到有序排列中进行判断。
>
> 如果不在的话，就证明target只可能落在另一排序中，将问题规模转变成规模更小的同类问题。
>
>  
>
> 题解中，一些部位的边界判断相当难受。如其中的
>
> ```java
>   if (nums[0] <= nums[mid]) {
>       if (nums[0] <= target && target < nums[mid]) {
...
> ```
>
> 其中的几个位置的>=、<=、<、>的限制相当严格，不能错位。
>
> 核心就是：
>
> 1. 判断有序排列时带=；
> 2. 判断是否属于有序排列中时，并集判断中mid的判断不带=；
> 3. l和r变化时都是从mid的相邻位置开始变化。



## 64	2025-02-13

### （1）寻找旋转排序数组中的最小元素——153
...
> 核心思路
>
> 这个题比较简单，正好能用到在上一题中，个人思路二中的前半部分中找offset的步骤作为解答。
...
> 核心思路
>
> 官方的题解看起来和自己的解答非常相似，但是其中的边界判断更加的精细，最终还能省去下方的判断。
>
> ```java
> if (nums[end] > nums[start]) {
>     return nums[start];
...
class MinStack {
     ...
     public void push(int val) {
          // 初始化一个新的stackNode
          MinStack newStackNode = new MinStack();
          newStackNode.val = val;

          // 如果当前的Stack是空的
          if (top == null) {
               // 将栈顶指针以及最小指针定位到当前节点即可退出
               top = newStackNode;
               minStack= newStackNode;
               return;
          }
          ...
          MinStack stackNode = newStackNode.next;
          MinStack lastStack = stackNode;

          // true 代表往大的方向上升，false 相反。
          if (newStackNode.val > newStackNode.next.val) {
               while(stackNode != null && val > stackNode.val) {
                    lastStack = stackNode;
                    stackNode = stackNode.val_higher;
               }
               newStackNode.val_higher = stackNode;
               newStackNode.val_lower = lastStack;

               if (newStackNode.val_higher != null){
                    newStackNode.val_higher.val_lower = newStackNode;
               }
               lastStack.val_higher = newStackNode;
          }else {
               while(stackNode != null && val <= stackNode.val) {
                    lastStack = stackNode;
                    stackNode = stackNode.val_lower;
               }
               newStackNode.val_lower = stackNode;
               newStackNode.val_higher = lastStack;

               if (newStackNode.val_lower != null){
                    newStackNode.val_lower.val_higher = newStackNode;
               }else {
                    minStack = newStackNode;
               }
               lastStack.val_lower = newStackNode;
          }
     }

     public void pop() {
          if (top.val_lower == null && top.val_higher != null){
               minStack = top.val_higher;
               top.val_higher.val_lower = null;
          }else if (top.val_lower != null && top.val_higher == null){
               top.val_lower.val_higher = null;
          }else if (top.val_lower != null && top.val_higher != null){
               top.val_higher.val_lower = top.val_lower;
               top.val_lower.val_higher = top.val_higher;
          }
          top = top.next;
     }
...
> 核心思路：
>
> ![image-20250214174130357](https://gitee.com/hanhandehanpi/cloudimage/raw/master/image-20250214174130357.png)
>
> 难度主要体现在push与MinStack数据结构的设计之中。
>
> 最后将MinStack设计为一个具备双向大小指针、栈等下一个指针、栈顶指针、以及最小节点指针的结构。
>
> 在push的时候需要考虑若干点：
>
> 1. 是否为空？
> 2. 是往哪个方向进行放置双向大小指针。
>
> 同时本题主要为了查找MinStack进行了优化，进行push时计算量较大。
...
//                    temp.append(stack.pop());
                ...
                for (int t = 0; t < Integer.parseInt(times.toString()); t++) {
...
> - 将所有的字符加入stack中；
> - 当且仅当出现`]`时进行额外处理：
>   1. 将stack中的内容抛出并记录在temp字符串中，当且仅当遇到`[`时，将temp重复`times`遍并再次加入stack中。
> - 最后将stack中的内容转成String类型的`res`，并作为结果返回。
>
> > 还有一个错误的实现：
> >
> > 这个错误实现关键问题在于，当涉及到多重嵌套时，无法储存那么多的待加入res的字符串。
> >
> > 原本试图使用`oneLoopOut`作为每次添加入res中的字符串内容，结果发现当多重嵌套+间隔【如`2[a2[b]cd[e]f]`，其中的间隔`cd`不好处理】时便无法将所有的数据存储
> >
> > ```java
> > class Solution {
> >     public String decodeString(String s) {
> >         StringBuilder res = new StringBuilder();
> >         Deque<Character> stack = new ArrayDeque<>();
> >         StringBuilder oneLoopOut = new StringBuilder();
> >         Boolean newTime = true;
> > 
> >         for (int i = 0; i < s.length(); i++) {
> >             char curr = s.charAt(i);
> >             if (curr >= 'a' && curr <= 'z' && newTime) {
> >                 oneLoopOut.append(curr);
> >             }else if (curr != ']'){
> >                 stack.push(curr);
> >                 newTime = false;
> >                 continue;
> >             }else {
> >                 StringBuilder temp = new StringBuilder(oneLoopOut);
> >                 while (stack.peek() != '['){
> >                     temp.insert(0, stack.pop());
> >                 }
> > 
...
> >                     times.insert(0, stack.pop());
> >                 }
> >                 oneLoopOut.append(String.valueOf(temp).repeat(Integer.parseInt(times.toString())));
> >                 newTime = true;
> >             }
> >             if (stack.isEmpty()){
> >                 res.append(oneLoopOut);
> >                 oneLoopOut = new StringBuilder();
> >             }
> >         }
> >         return res.toString();
> >     }
...
> 官方的题解的优势在于：
>
> 1. **用不定长数组来模拟栈操作，方便从栈底向栈顶遍历。**
>
>    最终不需要再额外遍历并倒置一遍stack中的内容作为`res`。
...
class Solution {
    public int[] dailyTemperatures(int[] temperatures) {
        ...
        for (int i = 0; i < temperatures.length; i ++) {
            ...
            if (!numsStack.isEmpty() && numsStack.peek() < curr) {
                do{
                    int temp = indexStack.pop();
                    res[temp] = i - temp;

                    numsStack.pop();
                }while (!numsStack.isEmpty() && numsStack.peek() < curr);
            }
            numsStack.push(curr);
            indexStack.push(i);
        }
...
> 设置两个栈，一个存数值，一个存位置。
>
> ![image-20250218151024898](https://gitee.com/hanhandehanpi/cloudimage/raw/master/image-20250218151024898.png)
...
> 核心思路
>
> 自己的实现中，其实numsStack是完全没必要的，毕竟既然已经有了indexStack，那么对应的值就是可以获取的。
>
> 然后自己的代码中还有可以优化的点：
>
> 1. if + do
>
>    这两者的效果其实就是while，不太应该写成这种效果。
>
> 2. Stack换成Deque
>
> 修改后的代码：
...
> - 设置一个范围足够大的桶数组；
>
> - 遍历nums数组，所有数出现一次便在其index位置使得该桶++；
>
> - 当需要桶的第k大的数时，从桶的最后往最前遍历；
>
>   当需要桶的第k小的数时，从桶的最前往最后遍历；
...
class Solution {
    public int[] topKFrequent(int[] nums, int k) {
        int [] buckets = new int[200000];
        int [] res = new int[k];
        for (int num : nums) {
            buckets[num - 1 + 100000]++;
        }
        PriorityQueue<Integer> minHeap = new PriorityQueue<>(k, (a, b) -> buckets[a + 100000] - buckets[b + 100000]);
        for (int i = 0; i < 200000; i++) {
            if (minHeap.size() < k) {
                minHeap.offer(i - 100000);
                continue;
            }
            if (buckets[minHeap.peek() + 100000] < buckets[i]) {
                minHeap.poll();
                minHeap.offer(i - 100000);
            }
        }
...
> 桶排序在涉及到频次时比较好用。
>
> 需要注意的点就是在堆中存储的是`索引值`，但是桶的排序是通过`buckets[索引值]`来进行的。
...
> 核心思路
>
> 挺快的一次解题，顺其自然的思路。没啥难点。
>
> ![image-20250220143750876](https://gitee.com/hanhandehanpi/cloudimage/raw/master/image-20250220143750876.png)
...
> 贪心。
>
> 以当前index = i为初始，往后遍历直到i < j <= i + nums[i]。
>
> 每次遍历到一个j点时，计算这个点的 j + nums[j]是否能大于最远可到达的位置，如果能就将当前的j点作为最远的方案。
>
> ![image-20250220151317148](https://gitee.com/hanhandehanpi/cloudimage/raw/master/image-20250220151317148.png)
...
> 核心思路
>
> 从设计的逻辑来看和自己的代码差不多，这个代码更优美，自己的代码更容易阅读理解。
...
class Solution {
    public List<Integer> partitionLabels(String s) {
        ...
        while (left < length) {
            ...
            // 找到与最优先的left时，将left ~ currMax之间的数全部加入set中
            for (int i = left; i <= currMax; i++){
                currSet.add(s.charAt(i));
            }

            // 之后再从后往前遍历，确认本次Set的最大边界。
            right = length - 1;
            while (right != currMax){
                // 如果存在相同的，那么就把最大边界更新，在重新循环。
                if (currSet.contains(s.charAt(right))) {
                    for (int i = currMax + 1; i <= right ; i++){
                        currSet.add(s.charAt(i));
                    }
                    currMax = right;
                    right = length;
            ...
            // 确认了本次set 的边界之后，清除set，重新开始一次循环。
            result.add(currMax + 1);
...
> - 最核心的一个思路就是
>
>   第一次遍历一遍数组，计算出其中每个字符出现的最后位置。
>
> - 第二个for循环也比较精妙
>
>   能确保在一遍循环之内找到当前的字符串的末尾同时不需要引入Set。
...
>         return rob(nums, 0);
>     }
>     public int rob(int[] nums, int curr) {
>         if (curr == nums.length ){
>             return 0;
>         }
...
>             return nums[curr];
>         }
>         return Math.max(rob(nums, curr + 2) + nums[curr], rob(nums, curr + 1));
>     }
> }
> ```
>
> 确实这会导致更多的没必要的计算。
>
> `Math.max(rob(nums, curr + 2) + nums[curr], rob(nums, curr + 1))`
>
> 主要是这部分内部的rob实际上进行了多次的重复计算，需要使用数组来减少计算量。
>
> 在知道了该使用数组来记录这个额外的重复计算的之后，实现了以下代码，虽然效率达到了需求，但是非常的不优美：
>
> ```java
> class Solution {
>     int [] count;
...
> 
>         if (curr == length - 2){
>             count[length - 2] = Math.max(nums[length - 1], nums[length - 2]);
>             return count[length - 2];
>         }
> 
>         count[curr] = Math.max(nums[curr] + rob(nums, curr + 2), rob(nums, curr + 1));
> 
>         return count[curr];
>     }
> }
...
> 核心思路
>
> 自己的代码丑陋之处在于，自己的想法是从后往前加资源，但是自己的curr是从0往length方向进行的。
>
> 官方的代码就是两个都是同方向的。就没有那么丑陋。
>
> 自己整理了一下思路后完成的下部分代码：
>
> ```java
...
> 
>         for (int i = 1; i < length; i++){
>             int maxCurr = Math.max(ahead, aheadOfAhead + nums[i]);
>             aheadOfAhead = ahead;
>             ahead = maxCurr;
>         }
>         return ahead;
...
class Solution {
    public int numSquares(int n) {
        ...
        for (int j = 1; j <= n ; j++){
            int currMin = 1001;
            for (int m = 1; m * m <= j; m++){
                currMin = Math.min(count[m * m] + count[j - m * m], currMin);
            }
            count[j] = currMin;
        }
        return count[n];
...
> 核心思路
>
> <img src="https://gitee.com/hanhandehanpi/cloudimage/raw/master/image-20250224215054760.png" alt="image-20250224215054760" style="zoom:33%;" />
>
> 构造一个count[]数组，其中存放每个位置的数的最小完全平方数。
>
> 在第一次初始化中，将所有的平方数位置的count值设置为1，如1 4 9 16 25。
>
> 第二次遍历时，从1~n进行遍历。
>
> 在每次遍历中，将当前的位置的数设置为一个最小值，这个最小值的判断方式为：
>
> 计算从1~k~j之中，出现的count[k]值会使得count[k] + count[j - k]比记录的最小值更小。
>
> Q：这是为什么呢？
>
> A：发现了一个规律，虽然并不确定是最小时间复杂度的处理方式以及没有排除一些合理的剪枝手段，但确实是一种正确的处理手段：
>
> ​	以23为例，其最终的结果应该是4。即9+9+4+1。
>
> ​	在此之前，已经获得了其前边一些的数的和。
>
> <img src="https://gitee.com/hanhandehanpi/cloudimage/raw/master/image-20250224214547848.png" alt="image-20250224214547848" style="zoom:33%;" />
>
> ​	此时，计算numS[1] + numS[23 - 1]即1 + 3 = 4；
>
> ​	便先将其值设置为4；
>
> ​	再看 numS[4] + numS[23 - 4]即1 + 3 = 4；
>
> ​	再看numS[9] + numS[23 - 9]即1 + 3 = 4；
>
> ​	再看numS[16] + numS[23 - 16]即1 + 4 = 5；
>
> ​	此时便可截止，最小值为4。
...
> 2. 省去了初始化设置1的步骤。
>
> 3. 在判小的步骤中，没必要计算count[m * m]，因为这个就是1。
>
>    ```java
>    currMin = Math.min(count[m * m] + count[j - m * m], currMin);
>    ```
...
>
>    ```java
>    currMin = Math.min(1 + count[j - m * m], currMin);
>    ```
>
> 修改完善之后，自己的代码变成：
>
...
> ```
>
> 奇怪的是，修改了之后的代码运行效率竟然比官方的还高：
>
> <img src="https://gitee.com/hanhandehanpi/cloudimage/raw/master/image-20250224220231268.png" alt="image-20250224220231268" style="zoom:33%;" />
>
> 非常的奇怪，本以为将
>
> ```java
...
> ```java
> for (int m = 1; m * m <= j; m++){
>     currMin = Math.min(count[j - m * m], currMin);
> }
> count[j] = currMin + 1;
> ```
>
...
class Solution {
    public int lengthOfLIS(int[] nums) {
        ...
        map.put(nums[0], 1);
        for (int i = 1; i < length; i++){
            int maxBeforeValue = Integer.MIN_VALUE;
            // 是否有比其更小的值
            boolean flag = false;
            for (int j : map.keySet()){
                if (j < nums[i] && maxBeforeValue < map.get(j)){
                    maxBeforeValue = map.get(j);
                    flag = true;
                }
            }
            if (flag){
                map.put(nums[i], maxBeforeValue + 1);
                res = Math.max(res, maxBeforeValue + 1);
            }else {
                map.put(nums[i], 1);
            }
        }
...
> 没想到更好的实现了。
>
> 再次的实现方式，就是通过两层循环，时间复杂度应该不会更好。
>
> 想了想，还是去实现了以下使用两个for循环，实际上的执行效率还会更好一点。
>
> ```java
> class Solution {
>     public int lengthOfLIS(int[] nums) {
...
>             for (int j = 0; j < i; j++){
>                 if (nums[j] < nums[i]){
>                     currMax = Math.max(currMax, refer[j] + 1);
>                 }
>             }
>             refer[i] = currMax;
...
> ```
>
> ![image-20250228152727886](https://gitee.com/hanhandehanpi/cloudimage/raw/master/image-20250228152727886.png)
>
> 貌似时间复杂度还行，代码也更简洁一点。
...
class Solution {
    public int maxProduct(int[] nums) {
        ...
        for (int i = 1; i < nums.length; i++){
            if (nums[i] >= 0){
                dpup[i] = Math.max(dpup[i - 1] * nums[i], nums[i]);
                dpdown[i] = dpdown[i - 1] * nums[i];
                max = Math.max(max, dpup[i]);
            }else {
                dpup[i] = dpdown[i - 1] * nums[i];
                dpdown[i] = Math.min(dpup[i - 1] * nums[i], nums[i]);
                max = Math.max(max, dpup[i]);
            }
        }
        return max;
...
> 核心思路：
>
> 构建一个dpup以及dpdown数组，分别用来存储在第i个位置时，包括此值及往前的乘积最大的值以及最小的值。
>
> 然后就是根据当前的值是正数还是负数进行对应处理。
>
> 需要注意的点：
>
> ```java
...
>
> ```java
> dpup[i] = dpup[i - 1] * nums[i];
> ```
>
> 理由是，反正nums[i]已经是>=0了，一定会有dpup[i - 1] * nums[i]比nums[i]大。
>
> 但是实际上并不是这样，因为会有情况使得nums[i]>=0，同时dpup[i - 1] <= 0；
>
> 可以使用[0,2]来推导一遍。
...
class Solution {
    public int maxProduct(int[] nums) {
        ...
        for (int i = 1; i < nums.length; i++){
            ...
            if (currValue >= 0){
                up = Math.max(up * currValue, currValue);
                down = down * currValue;
            }
            else {
                int temp = up;
                up = down * currValue;
                down = Math.min(temp * currValue, currValue);
            }
            max = Math.max(max, up);
        }
        return max;
...
> 是上边评论区题解中的优化版。
>
> ![image-20250228170848445](https://gitee.com/hanhandehanpi/cloudimage/raw/master/image-20250228170848445.png)
...
>             return false;
>         }
>         return findTarget(nums, sum / 2, 0);
>     }
>     public boolean findTarget(int[] nums, int target, int index){
>         if (target == 0){
>             return true;
>         }
...
>             return false;
>         }
>         if (findTarget(nums, target - nums[index], index + 1)){
>             return true;
>         }
>         if (findTarget(nums, target, index + 1)){
>             return true;
>         }
>         return false;
>     }
...
> ```
>
> 想着找两个和相同的子集，那就是相当于找是否存在和为所有元素之和的一半的子集。
>
> 但是第二步的寻找过程计算性能开销过大，无法满足需求，试了一些剪枝，但是无法满足需要。
...
> 核心思路
>
> 参考[官方题解](https://leetcode.cn/problems/partition-equal-subset-sum/solutions/442320/fen-ge-deng-he-zi-ji-by-leetcode-solution/?envType=study-plan-v2&envId=top-100-liked)。
...
> 核心思路：
>
> 规律还是比较容易找的，但是花了点时间想有没有不用构造这么大一个空间的方式，不过没有想到，遂直接使用了该方法。
...
> 核心思路：
>
> 不需要使用二维数组，只需要使用一维数组即可存储所有需要的数。
>
> <img src="https://gitee.com/hanhandehanpi/cloudimage/raw/master/image-20250301123229546.png" alt="image-20250301123229546" style="zoom: 33%;" />
>
> 一维数组这里如果不太好理解，可以借助上边的图，将每一列视为一个一个数组。【题解中是将一行视为一个数组，其实是一个道理】
>
> 如最开始nums[] = {1,1,1,1,1,1,1};
>
> 首先更新的是第一个数，这个数不会变动，对应图中的第一行，值永远不变。
>
> 新的第二个数 = 变化后的第一个数 + 原本的第二个数；
>
> 新的第三个数 = 变化后的第二个数 + 原本的第三个数；
>
> 以此类推。
>
> 即 `nums[i] = nums[i - 1] + nums[i];`也就是`nums[i] += nums[i - 1];`。
...
class Solution {
    public int minPathSum(int[][] grid) {
        ...
        int [][] far = new int[m][n];
        for (int i = 0; i < m; i++){
            Arrays.fill(far[i], Integer.MAX_VALUE);
        }
        far[0][0] = grid[0][0];
        for (int i = 0; i < m; i++){
            for (int j = 0; j < n; j++){
                if (i != 0 && j != 0){
                    far[i][j] = Math.min(far[i - 1][j] , far[i][j - 1]) + grid[i][j];
                }else if (i == 0 && j != 0){
                    far[i][j] = far[i][j - 1] + grid[i][j];
                }else if (i != 0 && j == 0){
                    far[i][j] = far[i - 1][j] + grid[i][j];
                }
            }
        }
        return far[m - 1][n - 1];
...
>         far [0] = grid[0][0];
>         for (int j = 1; j < n; j++){
>             far[j] = far[j - 1] + grid[0][j];
>         }
>         for (int i = 1; i < m; i++){
>             far[0] += grid[i][0];
>             for (int j = 1; j < n; j++){
>                 far[j] = Math.min(far[j - 1], far[j]) + grid[i][j];
>             }
>         }
>         return far[n - 1];
>     }
...
> ```
>
> 使用了一维数组替换了二位数组，理论上应该是减少空间比较多。但从测试结果看，消耗的内存不仅没减少，反而增加了一点点；反倒是执行效率提高了不少。
>
> 修改前<img src="https://gitee.com/hanhandehanpi/cloudimage/raw/master/image-20250301135302289.png" alt="image-20250301135302289" style="zoom:33%;" />
>
> 修改后<img src="https://gitee.com/hanhandehanpi/cloudimage/raw/master/image-20250301135026882.png" alt="image-20250301135026882" style="zoom:33%;" />
...
> 核心思路：
>
> 记录一个二维数组`dp[i][j]`，这个二维数组用来表示text1[i]与text2[j]的最长公共子序列。
>
> <img src="https://gitee.com/hanhandehanpi/cloudimage/raw/master/image-20250301151430753.png" alt="image-20250301151430753" style="zoom:33%;" />
>
> 以上图为例，其中`dp[i][0]`以及`dp[0][j]`全部默认为0；
>
> - 当遇到不同的时候，就从上边或者左边中取一个最大值，作为当前位置的dp值；
> - 当遇到相同的时候，就从左上角取值+1作为当前的值。
>
> Q：如何理解`dp[i][j]`所代表的含义？
>
> A：`dp[i][j]`代表的是，text1以及text2分别使用长度为i与j的前缀，能构成的最长公共子序列。
>
> 以图中的`dp[2][1]`为例，就代表字符串“abcbdab”去前2个即“ab”；字符串“bdcaba”取前一个即“b”所能构成的最长公共子序列。即“b”，因此`dp[2][1] = 1` 。
>
> Q：如果我不只是希望得到最大的公共子序列长度，我需要得到这个字符串本身，我需要怎么做？
>
> A：⭐
...
> ```
>
> 但我直接告诉我，当”A 和 B 的最后一个字母相同“时，`D[i][j] = D[i - 1][j - 1]`即可，而不用去计算与其他两个数的的min值。遂将源代码进行修改。
>
> 尝试在[评论区](https://leetcode.cn/problems/edit-distance/solutions/188223/bian-ji-ju-chi-by-leetcode-solution/)询问无果：
>
> 
>
> 修改后的题解也能通过官方测试：⭐
...
> 核心思路：
>
> 没啥难度。注意当count1 可能 = 0即可。
>
> 不过貌似不推荐这种实现，说是进行了两次遍历？
...
> 核心思路：
>
> 设置两个指针p0与p2。用来指示下一个0或者2该放置的位置。
>
> 需要注意的点是：
>
> - 在当nums[i] == 2这里，需要使用while进行循环判断而不是if。
> - 下边的if和上边的while并不是选一个执行，而是当while执行之后，还需要检查一次if == 0。
...
class Solution {
    public void nextPermutation(int[] nums) {
        ...
        int minBiggerThanleft = right;
        for (int i = right; i < length; i++){
            if (nums[i] > nums[left] && nums[i] < nums[minBiggerThanleft]){
                minBiggerThanleft = i;
            }
        }
        int temp = nums[left];
        nums[left] = nums[minBiggerThanleft];
        nums[minBiggerThanleft] = temp;
...
> 核心思路：
>
> 关键在于找到`nums[right] > nums[left]`。如果没找到就直接将数组重新排序【这里可以优化，不需要进行排序，只需要将数组倒转即可，因为此时数组一定是降序排序的】。
>
> 如果找到了，那就在left右边的数中，找到最小的大于`nums[left]`的数，将其与left换个位置。然后将left以右的数进行排序。
...
> 核心思路：
>
> 很好理解的思路。将数值作为index进行快慢指针跳转，然后使得当index重复时，即出现最终的环也就是结果。
>
> 这里需要注意的是，因为数组的值域是[1, n]，没有value = 0的存在，因此从index = 0开始查找不会出现因为`独立环`而产生的环。
>
> 评论区的一个很好的细节解释：
>
> ## 对方法 3 快慢指针解法的补充解释：为什么从`index 0`开始，一定会产生环？且环里一定包含重复数字？
>
> ### A. 一定存在环吗？
>
> 是的。因为index 范围是`[0,n]` value范围是`[1,n]`, 把value作为index一定可以不断的指向下一个位置，而数组容量有限，所以一定存在环。
>
...
>
> 但是如果从`index 1`开始，也形成了`2-1-2`这样一个环。这里没有重复数字也产生了环，可以把它叫做独立环。
>
> 结论 2：由于开始的位置不同，存在这两种不同类型的环：独立环和重复值环。
>
> ### C. 从`index 0`开始，产生的环里一定包含重复数字吗？
>
> 是的。因为value的值域`[1,n]`不包括0，所以`index 0`不能被其他值指向，所以`index 0`一定不会在某个环内，而是环外的一个节点。结合结论 1和结论 2，得知`index 0`如果要走到一个环中，**一定是因为重复数字的value都指向了独立环的一个节点(环起点)**，其中一个重复节点在环内，一个在环外。`index 0 `在索引若干次以后会指向在环外的那个重复值节点，于是形成了
>
> ```rust
> index 0 -> ... -> 重复值节点 -> 环起点 -> ...
> 
...
>
> 因此，如果修改题目为index 范围是`[0,n]` value范围是`[0,n-1]`, 则需要从 `index n `（value 中不存在的值）开始，才能找到重复数字，从其他位置开始，可能会形成不需要重复数字的独立环。
